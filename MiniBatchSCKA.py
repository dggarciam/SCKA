...
class MiniBatchCKA(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, showCommandLine = False, optimizer = 'Adam',
                 learning_rate=0.01,decay=0.1,min_lr=0.00001,beta_1=0.9,beta_2=0.999,epsilon=1e-7,
                 min_grad = 1e-5, epoch = 50, batch = 30,Q = 0.9, init = 'random', max_errpocket = 30, ort=True, reg=None, zeta=1):
        ...
        self.ort = ort # orthogonal solution
        self.reg = reg #'l2': l1-norm<zeta , 'l1': l1-norm<zeta
        self.zeta=zeta #regularization value              
...
#%% Train---------------------------------------------------------------    
    def proj1(self,V):
        '''
        V matrix of v_i columns i in N components, b>0
        '''
        b=self.zeta
        Vproj = np.zeros(np.shape(V))
        for i in np.arange(V.shape[1]):
            ...
            if np.linalg.norm(v,ord=1)<b:
                ...
                sv = np.cumsum(u)
                rho = np.where((u > (sv-b)/np.arange(1,np.shape(u)[0]+1)))[0][-1]
                theta = np.max([0, (sv[rho]-b)/(rho+1)])
                Vproj[:,i] = np.sign(v)*np.max([[np.abs(v)-theta],[np.zeros(np.shape(v))]],0)
        return Vproj
    def proj2(self,V):
        b=self.zeta
        Vproj = np.zeros(np.shape(V))
        for i in np.arange(V.shape[1]):
            v = V[:,i]
            if np.linalg.norm(v,ord=2)<b:
                Vproj[:,i]=v
            else:
                Vproj[:,i]=v/np.linalg.norm(v,ord=2)
        return Vproj
    ...
    def _ckatrain(self,X,L,labels): #X in N x P feature matrix, P features, N samples, L N x N target kernel,labels Nx1 vector
        ...
        Fcost =[]#    = np.zeros((epoch,1))#np.zeros((epoch*int(X.shape[0]/self.batch))) #cost along epoch
        ...
        for ii in np.arange(0,epoch):
            ...
            if batch<X.shape[0]:
                ...
                splt = sss.split(X,labels)
            else:
                splt = zip([np.arange(X.shape[0])],[np.zeros((X.shape[0],1))])                                    
            for train_index, test_index in splt:    
                ...
                if self.showCommandLine:
                    if batch==X.shape[0]:
                        ...
                        print('epoch:%d/%d--- fcostbest: %.7e --- fcost:%.7e --- elapsed:%.2f [s]\n' % (ii+1,epoch,fbest,fnew,time.time()-startt))
        A  = np.real(np.reshape(vbest,[np.shape(X)[1],-1],order='F'))                        	
        return A, np.array(Fcost), normA
